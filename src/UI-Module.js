...
const uiModule = (() => {
  ...
  const toggleNewTaskModal = () => newTaskModal.classList.toggle('toggle-display');

  const toggleExpandedModal = () => expandedModal.classList.toggle('toggle-display');

  const toggleEditModal = () => editModal.classList.toggle('toggle-display');
  ...
  const checkMarkStyling = (todo, toDoCheckBtn) => {
    if (todo.status === true) {
      toDoCheckBtn.classList.add('circle-checked');
  ...
  const removeActiveProject = () => {
    document.querySelectorAll('.btn-project').forEach((project) => project.classList.remove('active-btn'));
  };
